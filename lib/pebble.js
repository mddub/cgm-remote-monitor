...
function pebble (req, res) {
  ...
  function sendData () {
    ...
    if (sgvData.length > 0) {
        ...
        if (req.iob && sgvData[0].iob === undefined) {
              // calculate iob based on treatments if the most recent sgv datapoint does not contain iob
              ...
              if (!err && results) {
                results.forEach(function(element, index) {
                    if (element) {
                        ...
                        if (req.iob && 'iob' in element) {
                            obj.iob = element.iob.toFixed(2);
                        }
